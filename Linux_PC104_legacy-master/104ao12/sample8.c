...
int main(void)
{
   ...
   outportb(fd, baseAddr + 0x13 ,0x8b);        // send the control byte for A out B in
   outportb(fd, baseAddr + 0x18, 1);
   current[0] = 0;
   shift_left = 1;
   while(!kbhit())                    // loop until key pressed
      {
      outportb(fd, baseAddr + 0x10,current[0]);     // write value to port a
		usleep(10000); //delay for 0.01 seconds allowing card to settle

		inportb(fd, baseAddr + 0x10, &(current[1])); //read from port A
		inportb(fd, baseAddr + 0x11, &(current[2])); //read from port B
