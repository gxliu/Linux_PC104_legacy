...
int main (void)
{
	   ...
	   printf("=>");
		...
		switch (choice)
		{
			case 1:
				...
				if (restricted == 1)
				{
					outportb(fd, baseAddr + 0x10, 0x40); //clear the voltage restrict on the board
					restricted = 0;
				}
				break;
			case 2:
	...
	outportb(fd, baseAddr + 0x10, 0x0); //re enable the voltage restrict
...
void sendtoport()
{
   ...
   do
      ...
      for( i = 0;i < points;i++)
         {
         outport(fd, baseAddr + (dac*2), points_list[i]);
         }
      }
   while (!kbhit());
...
void sawcurve()
{
   ...
   printf("Generating saw tooth wave, press <enter> key to stop....\n");
...
// wave form slope
   ...
   printf("Generating triangle wave, press <enter> key to stop....\n");
	fflush(NULL); //be sure all the stuff gets printed before it get's stuck in the loop
   sendtoport();
}  // end trianglecurve
...
void sinecurve()
{
   ...
   printf("Generating sine wave, press <enter> key to stop....\n");
	fflush(NULL); //be sure all the stuff gets printed before it get's stuck in the loop
   sendtoport();
} // end sinecurve
